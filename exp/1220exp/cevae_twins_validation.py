...
class Config:
    # 数据路径 - Twins 数据集
    covariates_path = 'oringnal_data/bnlearn/twins/twins.csv'
    death_path = 'oringnal_data/bnlearn/twins/death.csv'
    confounder_path = 'outcome/1220_outcome/twins_final_data.json'  # LLM生成的混杂变量
    output_dir = 'outcome/1220_outcome/CEVAE_validation_twins'

    # 模型超参数
    x_dim = 15          # 协变量维度 (Twins数据集的15个关键协变量)
    z_dim = 5           # 隐变量维度 (Twins数据集较大,可以适当增加)
    hidden_dim = 128    # 隐藏层维度

    # 训练超参数
    batch_size = 128    # Twins数据集较大,可以用更大的batch
    learning_rate = 1e-3
    epochs = 200
...
class TwinsDataset:
    ...
    """
    def __init__(self, covariates_path, death_path, confounder_path=None):
    ...
    def _load_data(self):
        """从 CSV 文件加载数据"""
        print(f"加载协变量数据: {self.covariates_path}")
        print(f"加载死亡率数据: {self.death_path}")

        # 加载协变量数据
        df_covariates = pd.read_csv(self.covariates_path)
        # 加载死亡率数据
        df_death = pd.read_csv(self.death_path)
        ...
        # 提取变量
        self.X = df_twins[available_cols].values.astype(np.float32)
        self.T = df_twins['treat'].values.astype(np.float32).reshape(-1, 1)
        self.Y = df_twins['mort_1yr'].values.astype(np.float32).reshape(-1, 1)

        # 加载 LLM 生成的混杂变量 (如果提供)
        if self.confounder_path and os.path.exists(self.confounder_path):
            print(f"加载LLM生成的混杂变量: {self.confounder_path}")
            with open(self.confounder_path, 'r', encoding='utf-8') as f:
            ...
            # 从JSON中提取混杂变量值
            confounder_data = pd.DataFrame(data_dict.get('data', []))

            # 关键: JSON中的数据是采样后的子集,需要使用JSON中的完整数据
            # 而不是从原始CSV重新构建,否则会导致大小不匹配
            n_confounder_samples = len(confounder_data)
            print(f"JSON中的样本数: {n_confounder_samples}")

            if self.confounder_name in confounder_data.columns:
                # 使用JSON数据中的 X, T, Y, U (保证大小一致)
                # 重新从 JSON 数据中提取协变量
                self.X = confounder_data[available_cols].values.astype(np.float32)
                self.T = confounder_data['treat'].values.astype(np.float32).reshape(-1, 1)
                self.Y = confounder_data['mort_1yr'].values.astype(np.float32).reshape(-1, 1)
                self.U = confounder_data[self.confounder_name].values.astype(np.float32).reshape(-1, 1)

                # 更新 df_twins 用于后续统计
                df_twins = confounder_data
            else:
                print(f"警告: 混杂变量 {self.confounder_name} 不在数据中,使用随机值")
                self.U = np.random.randn(len(self.X), 1).astype(np.float32)
        else:
            # 如果没有混杂变量文件,生成随机占位符
            print("警告: 未找到混杂变量文件,使用随机占位符")
            self.confounder_name = 'random_placeholder'
            self.U = np.random.randn(len(self.X), 1).astype(np.float32)

        # 标准化
        self.X = self.scaler_X.fit_transform(self.X)
        # 注意: Y 是二元变量 (0/1),不需要标准化,但为了与模型兼容保留
        self.Y = self.scaler_Y.fit_transform(self.Y)
        self.U = self.scaler_U.fit_transform(self.U)

        print(f"最终数据集大小: {len(self.X)} 样本")
        print(f"协变量维度: {self.X.shape[1]}")
        print(f"处理组样本 (Heavier twin): {int(self.T.sum())}")
        print(f"对照组样本 (Lighter twin): {len(self.T) - int(self.T.sum())}")
        print(f"死亡率 - Lighter twin: {df_twins[df_twins['treat']==0]['mort_1yr'].mean():.4f}")
        print(f"死亡率 - Heavier twin: {df_twins[df_twins['treat']==1]['mort_1yr'].mean():.4f}")
...
def main():
    ...
    """
    print("\n" + "="*60)
    print("CEVAE 验证 LLM 生成混杂变量的实验 (Twins 数据集)")
    print("="*60)
    ...
    # Step 1: 加载数据 (Twins 数据集)
    print("\n--- Step 1: 加载 Twins 数据集 ---")
    dataset = TwinsDataset(
        covariates_path=config.covariates_path,
        death_path=config.death_path,
        confounder_path=config.confounder_path
    )
    ...
    # 对于 Twins 数据集: ATE = P(Y=1|T=1) - P(Y=1|T=0) (死亡率差异)
    ...
    print(f"Naive ATE (死亡率差异): {ate_naive:.6f}")
    ...
    # 反标准化 ATE (对于 Twins 数据集,Y 是二元的,所以 scale 可能接近1)
