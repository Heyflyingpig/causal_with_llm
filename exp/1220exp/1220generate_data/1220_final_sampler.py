...
def sample_from_distribution(record, confounder_name):
    ...
    Based on distribution type and parameters in a record, generate a random sample.
    """
    
    dist_type_key = f"{confounder_name}_distribution_type"
    ...
    # 指数分布
    elif any(keyword in dist_type for keyword in ["指数", "exponential"]):
        # 指数分布参数: lambda (rate parameter)
        # numpy使用scale = 1/lambda
        lambda_param = params.get("lambda", params.get("rate", 1.0))
        if lambda_param <= 0:
            print(f"警告: lambda参数必须为正数 ({lambda_param})。将使用默认值1.0。")
            lambda_param = 1.0
        scale = 1.0 / lambda_param
        sampled_value = np.random.exponential(scale=scale)
    
    elif any(keyword in dist_type for keyword in ["gamma"]):
        shape = params.get("shape", 1.0)
        scale = params.get("scale", 1.0)
        sampled_value = np.random.gamma(shape=shape, scale=scale)
    
    elif any(keyword in dist_type for keyword in ["lognormal"]):
        mean = params.get("mean", 0.0)
        std = params.get("std", 1.0)
        sampled_value = np.random.lognormal(mean=mean, sigma=std)
...
def main():
    ...
    Main function: Load JSON with distribution parameters, perform sampling, and save final dataset.
    """
    input_path = 'outcome/1220_outcome/twins_data_glm_output.json'
    output_path = 'outcome/1220_outcome/twins_final_data.json'
    ...
    for run_data in all_data:
        confounder_name = run_data.get("confounder_name", None)
        if not confounder_name:
            print("警告: 未找到confounder_name字段")
            continue

        print(f"\n处理混淆变量: {confounder_name}")
        print(f"分布类型: {run_data.get('distribution_type', 'Unknown')}")
        
        data_records = run_data.get("data", [])
        print(f"记录数量: {len(data_records)}")
        ...
        for i, record in enumerate(data_records):
            ...
            if isinstance(record.get(confounder_name), dict):
                ...
                print(f"  - 分布类型: {record.get(f'{confounder_name}_distribution_type', '未找到')}")
                print(f"  - 原始参数: {original_params}")
        
        print(f"✓ 成功采样 {success_count}/{len(data_records)} 条记录")

    # 保存处理后的完整数据
    with open(output_path, 'w', encoding='utf-8') as f:
        json.dump(all_data, f, indent=4, ensure_ascii=False)
        
    print(f"\n最终的采样数据集已成功保存到: {output_path}")
